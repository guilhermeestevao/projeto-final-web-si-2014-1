...
<body>
	<h1 style="text-align: center; margin-bottom: 30px;">Formulário de Dentista</h1>

	<form class="form-horizontal" name="form1" role="form"
		onsubmit="return VDentista();" action="adiciona" method="post"
		...
		<div class="form-group">
			...
			<div class="col-sm-2">
				<input id="estado" type="text" name="dentista.estado"
					class="form-control" value="${dentista.estado}" maxlength="2"/>
			</div>
			<label for="sexo" class="col-sm-1 control-label">Sexo:</label>
			<c:choose>
				<c:when test="${dentista.sexo == 'F'}">
					<label class="radio-inline"> <input id="sexo" type="radio"
						name="dentista.sexo" value="M" /> Masculino
					</label>
					<label class="radio-inline"> <input id="sexo" type="radio"
						name="dentista.sexo" value="F" checked="checked" /> Feminino
					</label>
				</c:when>
				<c:otherwise>
					<label class="radio-inline"> <input id="sexo" type="radio"
						name="dentista.sexo" value="M" checked="checked" /> Masculino
					</label>
					<label class="radio-inline"> <input id="sexo" type="radio"
						name="dentista.sexo" value="F" /> Feminino
					</label>
				</c:otherwise>
			</c:choose>
		</div>
		<div class="form-group">
			...
			<label for="nascimento" class="col-sm-2 control-label">Data
				de Nascimento:</label>
			<div class="col-sm-2">
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				<select id="assistente" name="dentista.assistente.id"
					class="form-control">
					<c:choose>
						<c:when test="${dentista.assistente.nome == null}">
							<option value="${dentista.assistente.id}" selected="selected"></option>
						</c:when>
						<c:otherwise>
							<option value="${dentista.assistente.id}"></option>
							<option value="${dentista.assistente.id}" selected="selected">${dentista.assistente.nome}</option>
						</c:otherwise>
					</c:choose>
			...
			<div class="col-sm-2">
				<input id="cro" type="text" name="dentista.cro" class="form-control"
					value="${dentista.cro}" />
			</div>
		</div>
		<div class="form-group">
